...
public final class CasinoLedgerDebugCommands {
    ...
    private static int fillLedger(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.translatable("command.cobblemoncasino.ledger_debug_filled", DEBUG_PLAYER_COUNT)
    ...
    private static int clearLedger(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.translatable("command.cobblemoncasino.ledger_debug_cleared", slotPlayers, blackjackPlayers)
                .withStyle(ChatFormatting.GREEN), true);
    ...
    private static int clearPlayer(CommandSourceStack source, String playerName) {
        ...
        if (playerId == null) {
            source.sendFailure(Component.translatable("command.cobblemoncasino.ledger_debug_unknown_player", playerName)
        ...
        if (!removedSlots && !removedBlackjack) {
            source.sendFailure(Component.translatable("command.cobblemoncasino.ledger_debug_no_stats", playerName)
                    .withStyle(ChatFormatting.RED));
            return 0;
        }

        source.sendSuccess(() -> Component.translatable("command.cobblemoncasino.ledger_debug_cleared_player", playerName)
                .withStyle(ChatFormatting.GREEN), true);
        return 1;
    }

    private static UUID debugUuid(int index) {
        String seed = "cobblemoncasino:DebugLedger:" + index;
