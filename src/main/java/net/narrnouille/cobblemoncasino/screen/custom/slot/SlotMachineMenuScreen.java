...
public class SlotMachineMenuScreen extends CasinoMachineScreen<SlotMachineMenuScreenHandler> {
    ...
    private void onPlusPressed() {
        if (minecraft != null && minecraft.player != null) {
            CobblemonCasinoPackets.sendToServer(new ChangeBetBaseC2SPayload(true));
        }
    }

    private void onSubtractPressed() {
        if (minecraft != null && minecraft.player != null) {
            CobblemonCasinoPackets.sendToServer(new ChangeBetBaseC2SPayload(false));
        }
    }
    ...
    private void sendMode(int mode) {
        if (minecraft != null && minecraft.player != null) {
            CobblemonCasinoPackets.sendToServer(new ChangeLinesModeC2SPayload(mode));
        }
    }
    ...
    protected void renderBg(GuiGraphics context, float delta, int mouseX, int mouseY) {
        ...
        context.blit(ModGuiTextures.CHIP, x + 53, y + 36, 0, 0, 12, 12, 12, 12);
    }

    @Override
    protected void renderLabels(GuiGraphics context, int mouseX, int mouseY) {
        context.drawString(font, Component.translatable("gui.cobblemoncasino.slot_machine.settings_bet_amount"),
                7, 25, 0xFFFFFF, true);
        context.drawString(font, Component.translatable("gui.cobblemoncasino.slot_machine.settings_lines"),
                7, 56, 0xFFFFFF, true);
        context.drawString(font, Component.translatable("gui.cobblemoncasino.slot_machine.settings_one_line"),
                23, 70, 0xFFFFFF, true);
        context.drawString(font, Component.translatable("gui.cobblemoncasino.slot_machine.settings_three_lines"),
                23, 85, 0xFFFFFF, true);
        context.drawString(font, Component.translatable("gui.cobblemoncasino.slot_machine.settings_five_lines"),
