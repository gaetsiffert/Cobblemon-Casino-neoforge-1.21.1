package net.narrnouille.cobblemoncasino.screen.custom.ledger;
...
public class CasinoLedgerScreen extends AbstractContainerScreen<CasinoLedgerScreenHandler> {
    ...
    protected void init() {
        ...
        this.slotsButton = new CommonButton(x + 10, y + 38, 82, 12, ModGuiTextures.BTN_LARGE,
                button -> setGame(LedgerGame.SLOTS), Component.translatable("gui.cobblemoncasino.casino_ledger.slots"));
        this.blackjackButton = new CommonButton(x + 10, y + 53, 82, 12, ModGuiTextures.BTN_LARGE,
                button -> setGame(LedgerGame.BLACKJACK), Component.translatable("gui.cobblemoncasino.casino_ledger.blackjack"));

        this.highestWinButton = new CommonButton(x + 10, y + 86, 82, 12, ModGuiTextures.BTN_LARGE,
                button -> setStat(LedgerStat.HIGHEST_WIN), Component.translatable("gui.cobblemoncasino.casino_ledger.highest_win"));
        this.totalWonButton = new CommonButton(x + 10, y + 101, 82, 12, ModGuiTextures.BTN_LARGE,
                button -> setStat(LedgerStat.TOTAL_WON), Component.translatable("gui.cobblemoncasino.casino_ledger.total_won"));
        this.totalLostButton = new CommonButton(x + 10, y + 116, 82, 12, ModGuiTextures.BTN_LARGE,
                button -> setStat(LedgerStat.TOTAL_LOST), Component.translatable("gui.cobblemoncasino.casino_ledger.total_lost"));
    ...
    protected void renderLabels(GuiGraphics context, int mouseX, int mouseY) {
        ...
        context.drawString(this.font, Component.translatable("gui.cobblemoncasino.casino_ledger.balance_label"),
                108, 31, 0xFFD96A, false);
        drawRightAligned(context, Component.translatable("gui.cobblemoncasino.chip_amount",
                TextUtils.formatWithCommas(data.balance())).getString(), 235, 42, 0xFFFFFF);

        context.drawString(this.font, Component.translatable("gui.cobblemoncasino.casino_ledger.game"), 10, 28, 0xFFD96A, false);
        context.drawString(this.font, Component.translatable("gui.cobblemoncasino.casino_ledger.record"), 10, 76, 0xFFD96A, false);

        context.drawString(this.font, Component.translatable("gui.cobblemoncasino.casino_ledger.your_stats"), 108, 56, 0xFFD96A, false);
        drawValue(context, "gui.cobblemoncasino.casino_ledger.highest_win", playerStats.highestWin(), 108, 68, false);
        drawValue(context, "gui.cobblemoncasino.casino_ledger.total_won", playerStats.totalWon(), 108, 79, false);
        drawValue(context, "gui.cobblemoncasino.casino_ledger.total_lost", playerStats.totalLost(), 108, 90, true);
        ...
        if (rows.isEmpty()) {
            context.drawString(this.font, Component.translatable("gui.cobblemoncasino.casino_ledger.no_records"),
    ...
    private List<FormattedCharSequence> leaderboardTitleLines() {
        return this.font.split(Component.translatable("gui.cobblemoncasino.casino_ledger.top",
    ...
    private enum LedgerGame {
        SLOTS("gui.cobblemoncasino.casino_ledger.slots") {
            @Override
            CasinoLedgerOpenData.GameStats stats(CasinoLedgerOpenData data) {
                return data.slots();
            }
        },
        BLACKJACK("gui.cobblemoncasino.casino_ledger.blackjack") {
            @Override
            CasinoLedgerOpenData.GameStats stats(CasinoLedgerOpenData data) {
    ...
    private enum LedgerStat {
        HIGHEST_WIN("gui.cobblemoncasino.casino_ledger.highest_win") {
            @Override
            List<CasinoLedgerOpenData.LeaderboardRow> rows(CasinoLedgerOpenData.GameStats stats) {
                return stats.highestWin();
            }
        },
        TOTAL_WON("gui.cobblemoncasino.casino_ledger.total_won") {
            @Override
            List<CasinoLedgerOpenData.LeaderboardRow> rows(CasinoLedgerOpenData.GameStats stats) {
                return stats.totalWon();
            }
        },
        TOTAL_LOST("gui.cobblemoncasino.casino_ledger.total_lost") {
            @Override
            List<CasinoLedgerOpenData.LeaderboardRow> rows(CasinoLedgerOpenData.GameStats stats) {
