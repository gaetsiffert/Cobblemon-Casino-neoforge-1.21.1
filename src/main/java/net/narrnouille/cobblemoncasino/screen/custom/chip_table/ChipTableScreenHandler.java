...
import net.narrnouille.cobblemoncasino.inventory.WalletInventory;
...
import net.narrnouille.cobblemoncasino.screen.widget.BetSlot;
...
public class ChipTableScreenHandler extends AbstractContainerMenu {
    ...
    public void convert(Player player, ChipTableConversionMode mode, long cobbledollarAmount) {
        ...
        if (!isContainerEmpty(this.outputInventory)) {
            CobblemonCasinoLogger.toPlayerTranslated(player, "message.cobblemoncasino.chip_table_output_not_empty", false);
            return;
        }
        ...
        if (!canFitOutput(result.outputs())) {
            CobblemonCasinoLogger.toPlayerTranslated(player, "message.cobblemoncasino.chip_table_output_full", false);
            return;
        }
    ...
    private boolean applyCobbledollarDelta(Player player, long delta) {
        ...
        if (!applied) {
            CobblemonCasinoLogger.toPlayerTranslated(player,
                    delta > 0
                            ? "message.cobblemoncasino.chip_table_cobbledollars_unavailable"
                            : "message.cobblemoncasino.chip_table_cobbledollars_insufficient",
                    false);
        }
        return applied;
    }
