package net.narrnouille.cobblemoncasino.screen.custom.blackjack;
...
import net.narrnouille.cobblemoncasino.games.blackjack.BlackjackRules;
...
public class BlackjackTableScreen extends CasinoMachineScreen<BlackjackTableScreenHandler> {
    ...
    private void sendAction(BlackjackAction action) {
        ...
        CobblemonCasinoPackets.sendToServer(new BlackjackActionC2SPayload(
                this.menu.getMachinePos(), this.menu.getMachineKey(), action));
    }

    private void onPlusPressed() {
        if (minecraft != null && minecraft.player != null) {
            CobblemonCasinoPackets.sendToServer(new ChangeBlackjackBetIndexC2SPayload(true));
        }
    }

    private void onSubtractPressed() {
        if (minecraft != null && minecraft.player != null) {
            CobblemonCasinoPackets.sendToServer(new ChangeBlackjackBetIndexC2SPayload(false));
        }
    }
    ...
    protected void renderLabels(GuiGraphics ctx, int mouseX, int mouseY) {
        ...
        // ===== LABELS =====
        // Bet Amount
        drawCenteredTextInBox(ctx, Component.translatable("gui.cobblemoncasino.blackjack_table.bet_amount").getString(),
                85, 7, 155, 14, 0xFFFFFF);
        // Dealer's Hand
        drawCenteredTextInBox(ctx, Component.translatable("gui.cobblemoncasino.blackjack_table.dealers_hand", dealerValueText).getString(),
                53, 46, 188, 53, 0xFFFFFF);
        // Player's Hand
        drawCenteredTextInBox(ctx, Component.translatable("gui.cobblemoncasino.blackjack_table.players_hand", playerValueText).getString(),
                15, 143, 150, 150, 0xFFFFFF);
        // Balance
        drawCenteredTextInBox(ctx, Component.translatable("gui.cobblemoncasino.blackjack_table.balance").getString(),
                15, 209, 64, 216, 0xFFFFFF);
        // Last Win
        drawCenteredTextInBox(ctx, Component.translatable("gui.cobblemoncasino.blackjack_table.last_win").getString(),
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (isBusy() && minecraft != null && minecraft.player != null) {
            if (keyCode == 256 /* GLFW.GLFW_KEY_ESCAPE */) {
                CobblemonCasinoLogger.toPlayerTranslated(minecraft.player, "gui.cobblemoncasino.blackjack.esc", true);
                return true;
            }
    ...
    private void showResultText(long bet, long payout) {
        if (payout > bet) {
            resultText = Component.translatable("gui.cobblemoncasino.blackjack.win");
            resultColor = 0xFFFFFF;
        } else if (payout == bet && bet > 0) {
            resultText = Component.translatable("gui.cobblemoncasino.blackjack.draw");
            resultColor = 0xAAAAAA;
        } else {
            resultText = Component.translatable("gui.cobblemoncasino.blackjack.lose");
