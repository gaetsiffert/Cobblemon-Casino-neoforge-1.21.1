...
import net.narrnouille.cobblemoncasino.screen.layout.DancingClefairy;
import net.narrnouille.cobblemoncasino.screen.layout.SlotLineSprite;
...
public class SlotMachineScreen extends CasinoMachineScreen<SlotMachineScreenHandler> {
    ...
    private void onSpinPressed() {
        if (minecraft != null && minecraft.player != null && !isSpinning) {
            CobblemonCasinoPackets.sendToServer(new DoSpinC2SPayload());
        }
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (isBusy() && minecraft != null && minecraft.player != null) {
            if (keyCode == 256 /* GLFW.GLFW_KEY_ESCAPE */) {
                CobblemonCasinoLogger.toPlayerTranslated(minecraft.player, "gui.cobblemoncasino.slot_machine.esc", true);
                return true;
            }
    ...
    private void drawBetAmount(GuiGraphics context) {
        ...
        context.drawString(font, Component.translatable("gui.cobblemoncasino.slot_machine.bet_amount"), 60, 36, 0xFFFFFF, true);
    }
    ...
    private void debugCheckSpin(SlotSymbol[][] matrix, int stop1, int stop2, int stop3) {
        if (!net.narrnouille.cobblemoncasino.games.slot.SlotClientSynced.DEBUG) return;
        if (minecraft == null || minecraft.player == null) return;
