...
public class CobblemonUtils {
    ...
    public static PokemonProperties safeParse(String input, ServerPlayer player, MinecraftServer server) {
        ...
        if (properties == null) {
            CobblemonCasinoLogger.toPlayerTranslated(player, "message.cobblemoncasino.species_not_found", false, input);
            String messageToOps = "[Cobblemon] Player " + player.getName().getString()
                    + " tried to claim " + input + " but failed! (species not found)";
            CobblemonCasinoLogger.toOps(server, CobblemonCasinoLogger.LogLevel.ERROR, messageToOps);
            return null;
        }
        return properties;
    }
    ...
    public static PokemonProperties tryParse(String input) {
        ...
        if (species == null || !stripNamespace(species).equalsIgnoreCase(expectedSpecies)) {
            CobblemonCasino.LOGGER.warn("[Cobblemon] Invalid Pokémon string: {}", input);
            return null;
        }
        return pokemon;
    }
    ...
    public static void addPokemon(PokemonProperties properties, ServerPlayer player) {
        ...
        if (added) {
            CobblemonCasinoLogger.toPlayerTranslated(player, "message.cobblemoncasino.pokemon_received_party", true, pokemonName);
        } else {
            CobblemonCasinoLogger.toPlayerTranslated(player, "message.cobblemoncasino.pokemon_box_full", true, pokemonName);
            CobblemonCasino.LOGGER.warn("[Cobblemon] Player {} tried to claim Pokémon ({}) but has no space in PC!", player.getName().getString(), pokemonName);
        }
    }
